...
func TestCollectionGet(t *testing.T) {
	...
	if assert.Equal(t, 200, resp.StatusCode, "鉴权失败！") {
		resp, err = client.Get("http://127.0.0.1:9001/authv1/problem/collection/get?problem_ids=1,2,3,4")
		if err != nil {
...
func TestCollectionSet(t *testing.T) {
	...
	defer resp.Body.Close()
	if assert.Equal(t, 200, resp.StatusCode, "鉴权失败！") {
		resp, err := client.Post("http://127.0.0.1:9001/authv1/problem/collection/set",
			"application/x-www-form-urlencoded",
			strings.NewReader("problem_id=1&flag=cancel"))
		if err != nil {
			fmt.Println("POST请求失败: " + err.Error())
		}
